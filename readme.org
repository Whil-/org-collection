...
- :customization :: Value is an association list of org customizations
  ...
  starting with =org-=, =ol-= and =ox-= are allowed here. Note that
  quoting isn't supported here, as this list is seen as data not to be
  evaluated. No evaluation means no quoting. [fn:1]
...
org-collections (to make sure changes take effect).
...
file that by default is stored inside your emacs customization folder,
called =org-collections=.
