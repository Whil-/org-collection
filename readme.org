...
whole of emacs and a singluar Org mode file. This hole is intended to
be filled with the concept of an Org collection.

* Installation
Package currently only exist on Github and not in (M)ELPA. But using
(for example) use-package and straight something like this will load
this package:
...
nothing on it's own but provies a couple of functions which can be
used to load, unload and switch between collections.

Notably the following:
- =org-collection-register-and-lock=
- =org-collection-lock=
...
At the time of writing this there are no helpers or validators
available for creating this. Sorry.
...
The main way of activating a collection is by locking it in place.
Functions for this are:
- =org-collection-lock=
- =org-collection-unlock=
...
This will also enable =org-collection-lock= for this collection for
the future.

When a lock is enabled the mode-line for org-collection-mode will show
=OC!:collection name= to signal that a collection is locked in place.
